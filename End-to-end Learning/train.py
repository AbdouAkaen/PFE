...
val_losses = []
"""
*  Function to train the model with the input data and save them.
...
def train_model(args, model, dataset_train, dataset_val):
    ...
    for epoch in range(args.nb_epoch): # runs for the number of eposchs set in the arguments
        ...
        for i, sample_id in enumerate(sampler):
            ...
            # We could've used the 'augument' function instead of the four coming rows
            ...
            if step%5000==0:
                ...
                val_loss = eval_model(model,dataset_val, num_samples=5000)
...
"""
*  Function to evaluate the model generated by the training process
...
"""
*  The main file to run the training of the dataset and generate the model.
...
def main(args):
    ...
    plt.savefig('Images/train_loss(%d,%f,%f).png'%(args.nb_epoch,args.learning_rate,args.keep_prob))
    plt.clf()
    # plt.show()
    ...
    plt.savefig('Images/val_loss(%d,%f,%f).png'%(args.nb_epoch,args.learning_rate,args.keep_prob))
    # plt.show()
    plt.clf()

"""
*  Runs the main function and gets the arguments from the user or 
